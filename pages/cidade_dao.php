...
//                    } 
                ...
                case 'listar_cidade' : {
                    try {
                        $file = '../logs/cidades_' . $usuario->getToken_id() . '.json';
                        if (file_exists($file)) {
                            unlink($file);
                        }
                        ...
                        $sql  = "Select ";
                        $sql .= "    c.cd_cidade ";
                        $sql .= "  , c.nm_cidade || ' (' || e.uf_estado || ')' as nm_cidade ";
                        $sql .= "from SYS_CIDADES c ";
                        $sql .= "  inner join SYS_ESTADOS e on (e.cd_estado = c.cd_estado) ";
                        $sql .= "where (c.cd_estado = {$cd_estado}) ";
                        
                        $res = $pdo->query($sql);
                        
                        $registros = array('cidades' => array());
                        $i = 0;
                        
                        while (($obj = $res->fetch(PDO::FETCH_OBJ)) !== false) {
                            $registros['cidades'][$i]['cd_cidade'] = $obj->cd_cidade;
                            $registros['cidades'][$i]['nm_cidade'] = $obj->nm_cidade;
                            $i += 1;
                        }
