...
//                        $html .= "        </div>";
                ...
                case 'listar_bairro' : {
                    try {
                        $file = '../logs/bairros_' . $usuario->getToken_id() . '.json';
                        if (file_exists($file)) {
                            unlink($file);
                        }
                        ...
                        $sql .= "  , b.nm_bairro ";
                        $sql .= "from SYS_BAIRROS b ";
                        $sql .= "where (b.cd_cidade = {$cd_cidade}) ";
                        
                        $res = $pdo->query($sql);
                        
                        $registros = array('bairros' => array());
                        $i = 0;
                        
                        while (($obj = $res->fetch(PDO::FETCH_OBJ)) !== false) {
                            $registros['bairros'][$i]['cd_bairro'] = $obj->cd_bairro;
                            $registros['bairros'][$i]['nm_bairro'] = $obj->nm_bairro;
                            $i += 1;
                        }
